  ...
  async init() {
    ...
    try {
      ...
      this.sock.ev.on('group-participants.update', async (update) => {
        ...
        if (action === 'add' && this.welcomeGroups.has(id)) {
            ...
            await this.sock?.sendMessage(id, {
              ...
              contextInfo: {
                ...
                forwardedNewsletterMessageInfo: {
                  ...
                  serverMessageId: 1,
                  newsletterName: 'MIN-XD BOT UPDATES'
                }
              }
            });
          }
        } else if (action === 'remove' && this.goodbyeGroups.has(id)) {
          for (const user of participants) {
            const userId = typeof user === 'string' ? user : (user as any).id;
      ...
      this.sock.ev.on('messages.upsert', async (m) => {
        ...
        // Activity Simulation (Typing/Recording)
        if (!isMe) {
          const activity = Math.random() > 0.5 ? 'composing' : 'recording';
...
*╰──────────────────╯*`;
        ...
        if (command === prefix + 'vv') {
          ...
          if (quoted) {
            ...
            if (viewOnce) {
              ...
              await this.sock?.sendMessage(remoteJid, { text: `*🌸 Oups !* Ce n'est pas un message à vue unique. ✨` }, { quoted: msg });
            }
          } else {
            await this.sock?.sendMessage(remoteJid, { text: `*🌸 Oups !* Réponds à un message à vue unique pour que je puisse le récupérer. 🎀` }, { quoted: msg });
          }
        }
        ...
        if (command === prefix + 'toimg' || command === prefix + 'tovideo') {
          ...
          if (stickerMsg) {
            const buffer = await downloadMediaMessage(
              { key: { remoteJid, id: contextInfo.stanzaId, participant: contextInfo.participant }, message: quoted } as any,
              'buffer',
              {},
              { logger: pino({ level: 'silent' }) } as any
            );
        ...
        if (command.startsWith(prefix + 'toimg')) {
          const users = msg.message.extendedTextMessage?.contextInfo?.mentionedJid || [];
